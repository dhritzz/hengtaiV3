...
C_BoStress-Testing-Toolkit by Pushy (c) | MIT 2020
Based on the CLIF Framework by Pushy (c) | MIT 2020C_W

BY USING THIS SOFTWARE, YOU MUST AGREE TO TAKE FULL RESPONSIBILITY
FOR ANY DAMAGE CAUSED BY Hengtai.
Hengtai SHOULD NOT SUGGEST PEOPLE TO PERFORM ILLEGAL ACTIVITIES.
C_Bo-----------------------------------------------------------C_W""")
	...
	def _add_commands(self):
		...
		event.help(["exit", "quit", "e", "q"], "Exit hengtai.")
		event.help("help", "View all commands.")
		event.help("upgrade", "Upgrade hengtai.")
		...
		event.help("ddos", "Connect to a hengtai server.")
		event.help_comment("\nModules:")
	...
	def upgrade():
		try:
			...
			print("[i] Deleting the temporary stored hengtai...")
			system("sudo rm -rf -i /tmp/hengtai/")
			var.stop()
			quit()
		except Exception:
			print("Cound not upgrade hengtai. (Make sure to use sudo)")
